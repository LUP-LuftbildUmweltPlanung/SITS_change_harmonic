...
def plot_timeseries(tsi_time_series, tss_time_series, threshold, points, with_std, save_fig, ylab, title, id_column):
    ...
    #print(dates)
    #print(tsi_time_series)

    # Filter out None or non-numeric values from tsi_time_series
    filtered_tsi_time_series = [(d, v) for d, v in tsi_time_series if v is not None and isinstance(v[0], (int, float))]

    if not dates or not filtered_tsi_time_series:
        print("No valid data points available for interpolation.")
        return  # Exit the function if there's nothing to plot or interpolate
    ...
    interpolated_tsi = np.interp([d.toordinal() for d in dates],
                                 [d.toordinal() for d in [ts[0] for ts in filtered_tsi_time_series]],
                                 [ts[1][0] for ts in filtered_tsi_time_series])
    ...
    time_series_threshold_plus = [[tsi_time_series[i][0], tsi_time_series[i][1][0] + threshold] for i in
                                  range(len(tsi_time_series))]

    time_series_threshold_minus = [[tsi_time_series[i][0], tsi_time_series[i][1][0] - threshold] for i in
                                   range(len(tsi_time_series))]

    plt.plot([tsi_time_series[j][0] for j in range(len(tsi_time_series))],
             [tsi_time_series[i][1] for i in range(len(tsi_time_series))], label=f"Harmonic", color='black',
             linewidth=1)

    if with_std:
        plt.plot([time_series_threshold_plus[j][0] for j in range(len(time_series_threshold_plus))],
                 [time_series_threshold_plus[i][1] for i in range(len(time_series_threshold_plus))], color='black',
                 label=f"Threshold", linewidth=0.5, linestyle='dashed')
    else:
        plt.plot([time_series_threshold_plus[j][0] for j in range(len(time_series_threshold_plus))],
                 [time_series_threshold_plus[i][1] for i in range(len(time_series_threshold_plus))], color='black',
                 label=f"Threshold: {threshold}", linewidth=0.5, linestyle='dashed')

    plt.plot([time_series_threshold_minus[j][0] for j in range(len(time_series_threshold_minus))],
             [time_series_threshold_minus[i][1] for i in range(len(time_series_threshold_minus))], color='black',
    ...
    for i in range(len(tss_time_series)):
        ...
        if isinstance(tss_time_series[i][1][0], (float, int)) and np.isnan(tss_time_series[i][1][0]):
            continue
        elif tss_time_series[i][1][0] < time_series_threshold_minus[i][1]:
            y_counter = y_counter +1
            if not display_anomaly:
                plt.scatter(tss_time_series[i][0], tss_time_series[i][1], color='orange', label=f"Anomaly",s=8)
                display_anomaly = True
            else:
                if y_counter >= 3:
                    if not display_disturbance:
                        plt.scatter(tss_time_series[i][0], tss_time_series[i][1], color='red', label=f"Disturbance",s=8)
            ...
            if not display_tss:
                ...
                plt.scatter(tss_time_series[i][0], tss_time_series[i][1], color='green', label=f"Observation", s=8)
                display_tss = True
            else:
    ...
    # Access the value in the `points` Series or DataFrame
    print(points)
    try:
        pid = str(points["id"].iloc[0])  # If `points` is a DataFrame and `id_column` is a column
    except AttributeError:
        pid = str(points.get("id", "Unknown"))  # If `points` is a Series and `id_column` is a key
